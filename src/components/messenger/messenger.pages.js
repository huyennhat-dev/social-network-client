...
const initFile = { url: "", public_id: "", name: "" };
...
const Message = () => {
  ...
  // upload image to cloudinary
  const handleUpFileToCloud = async () => {
    try {
      const { data } = await autoFetch.post(`/api/post/upload-file`, formData);
  ...
  const handleSendMess = async (receivedId) => {
    ...
    try {
      ...
      if (fileUrl.url) {
        fileUrl = await handleUpFileToCloud();
        if (!fileUrl.url) {
          setLoading(true);
  ...
  const handleFile = (e) => {
    ...
    const file = e.target.files[0];
    // @ts-ignore
    setFileData({ url: URL.createObjectURL(file), name: file.name });
          ...
          {!state.allMessages.length && !state.isNewMessage ? (
            ...
            >
              <div className="w-full rounded-full flex gap-x-2 items-center relative ">
                ...
                {fileData?.url && (
                  <div className="absolute w-[100px] h-[50px] md:w-[200px] md:h-[70px] rounded-md dark:bg-[#3e4042] border dark:border-white/50 top-[-50px] md:top-[-95px] right-[60px] z-[20] flex items-center justify-center bg-[#FFF] border-[#333]/70 ">
                    {state.loading && (
                      <div className="absolute z-[21] bg-black/50 w-full h-full flex items-center justify-center ">
                        <ReactLoading
                          type="spin"
                    ...
                    <AiOutlineFile className="mx-2 shrink-0 text-xl transition-50 opacity-60 hover:opacity-100 dark:text-[#b0b3b8] cursor-pointer " />
                ...
                />
                <label>
                  <AiOutlineFile className="shrink-0 text-xl transition-50 opacity-60 hover:opacity-100 dark:text-[#b0b3b8] cursor-pointer " />
                  <input
                    onChange={handleFile}
                    type="file"
                    accept=".doc, .docx, .pdf, .xls, .xlsx, .pptx"
                    name="file"
                    hidden
                  />
                </label>
