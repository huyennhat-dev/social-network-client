...
const initWeather = {
  ...
  clouds: { all: 0 },
  cod: 0,
  coord: { lon: 108.2208, lat: 16.0678 },
  dt: 0,
  id: 0,
  main: {
    feels_like: 0,
...
const Left = ({ dark, autoFetch }) => {
  ...
  const [cityName, setCityName] = useState("Đà Nẵng");
    ...
    if (weather.cod === 200) {
      return (
        <>
          ...
          <div className="my-1 text-center ">{weather.weather[0].main}</div>
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-x-2 gap-y-4 w-full mt-5 pt-5 border-t-[1px] dark:border-white/30 border-black/30 ">
            <div className="weather-information">
              <h1>Sunrise</h1>
              <p>{moment.unix(weather.sys.sunrise).format("H:mm")}</p>
            </div>
            <div className="weather-information">
              <h1>Sunset</h1>
              <p>{moment.unix(weather.sys.sunset).format("H:mm")}</p>
            </div>
            <div className="weather-information">
              <h1>Humidity</h1>
